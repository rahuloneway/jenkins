			...
			foreach($items as $item){ 
					...
					<tr data-toggle="collapse" data-target="#demo<?php echo $count; ?>" data-parent="#account-table" class="blue-accordian  collapsed <?php echo $class; ?> border-bottom">
						...
						<td data-title="Type " class="wid-15 ">
							...
							<?php echo getCustomerNameTBListing($primary["type_name"]); ?>
						</td>
						<td class="wid-10 ">
									...
									foreach( $item as $childs){ 
										...
										<tr class="border-bottom <?php echo  $class; ?>" >
											...
											<td data-title="Type"  class="wid-15 " >
												...
												<?php $catType = getCategoryType($childs["type_name"]); ?>
												<?php if($catType == 'CUSTOMERS') { 
													echo getCustomerNameTBListing($childs["type_name"]); 
												} else if($catType == 'SUPPLIERS'){ 
													echo getSupplierNameTBListing($childs["type_name"]);  
												}else if($catType == 'SHAREHOLDERS'){ 
													echo getShareholderNameTBListing($childs["type_name"]); 
												}else if($catType == 'EMPLOYEES'){ 
													echo getEmployeeNameTBListing($childs["type_name"]); 
												}else if($catType == 'BANK'){ 
													echo getBankNameTBListing($childs["type_name"]); 
												}else{
													echo $childs["type_name"]; 
												}?>
											</td>
