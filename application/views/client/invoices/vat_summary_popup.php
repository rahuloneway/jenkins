	...
	<form name="vat_dymanic_data" method="post">
		<table class="table table-striped">	
			...
			<tr>
				...
				<td>			
					<input type="text" name="total_val_due_period_ec_mem" id="total_val_due_period_ec_mem" class="validNumber total_val_due_period_ec_mem gess" value="<?php echo round($total_inv_due_vat,2); ?>" readonly />				
				</td>
			</tr>
			<tr>
				<td>
			...
			<td colspan="3">
				...
				<div class="vat_due">
					...
					<?php 						
						if ($PaidVatQuarters[$quarter_id]->accountant_submit == 1) { ?>
							...
							<?php
							} else if($PaidVatQuarters[$quarter_id]->vatDueRequest == 1 || $PaidVatQuarters[$quarter_id]->vatDueRequest == ''){ ?>
							<input type="button" style="display:none" name="submit_all_cal" class="submit_all_cal btn btn-info btn-xs color" 
                            value="SUBMIT ONLINE"/>
                            <input type="button" name="before_submit" class="before_submit btn btn-info btn-xs color" value="SUBMIT ONLINE"/>
					<?php } ?>
					</div>
				</div>
				<?php } 
