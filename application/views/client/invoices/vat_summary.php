...
//echo '<pre>';print_r($user);echo '</pre>'; die(' line 6,view-client-invoices-vat_summary.php');
...
// echo '<pre>';print_r($quaters);echo '</pre>';
...
if (!empty($VATitems) && count($VATitems) > 0) {
    foreach ($VATitems as $key => $val) {
        ...
        if (strtotime($date) >= strtotime($quaters[1]['FIRST']) && strtotime($date) <= strtotime($quaters[1]['SECOND'])) {
			...
			//echo "<pre>";print_r($PaidVatQuarters[1]); echo "</pre>";
            ...
            $total_vat[4] += $val->FlatRate;
        }
		
		//echo "<pre>"; print_r($total_sales); echo "</pre>";
...
<table class="table table-striped" >
    <thead>
        <tr>
			...
			<th><?php echo $this->lang->line('INVOICE_VAT_REQUEST_STATUS'); ?></th>
            <th><?php echo $this->lang->line('INVOICE_VAT_COLUMN_ACTION'); ?></th>
        </tr>
    </thead>
    <tbody>
        <?php //echo '<pre>';print_r($quaters);echo '</pre>'; //die();
        ...
        if (!empty($quaters)) {
            foreach ($quaters as $key => $val) {
                ...
                <tr>
					...
					<td data-title="Status">
						<?php if($total_sales[$x] == 0){ ?>
						<span class="btn btn-success btn-xs color" data-toggle="tooltip" data-placement="right" title="VAT Due Request Accepted">
								Accepted                            
							</span>						
						<?php } else if($PaidVatQuarters[$x]->vatDueRequest == '2'){ ?>
							<span class="btn label-danger btn-xs color" data-toggle="tooltip" data-placement="right" title="VAT Due Request Rejected">
								Rejected                            
							</span>
						<?php }else if($PaidVatQuarters[$x]->vatDueRequest == '1') { ?>
							<span class="btn btn-success btn-xs color" data-toggle="tooltip" data-placement="right" title="VAT Due Request Accepted">
								Accepted                            
							</span>
						<?php } else{?>							
							<span class="btn btn-info btn-xs color" data-toggle="tooltip" 
							data-placement="right" title="VAT Due Request Waiting" >
								Waiting
							</span>
						<?php } ?>
					</td>
