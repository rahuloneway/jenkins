...
<script>
	...
	jQuery('body').on('dblclick','#paidDate,.sdatepicker','.exDatepicker',function($){	
			//alert('test');			 
    ...
    $(document).on('focus', '#paidDate,.sdatepicker',function () {
		 $('#paidDate,.sdatepicker').datepicker({
			// $('#paidDate,.sdatepicker').datepicker("show")({
            dateFormat: 'dd-mm-yy',
            changeMonth: true,
            changeYear: true,
            yearRange: "-50:+1",
    ...
    {      
        ...
        $('.TotalMiles').each(function (e, i) {
            if ($(this).closest('tr').find('.ExpenseMileage option:selected').text() == 'Bike')
            {
				...
				//mileageTotalAmt = $('.MileageExpensed').val();
				...
				if (vech != 'v' && mileageAmt != '')
                {
					...
					$.each( arr, function( key, value ) {
						...
						//if(value.date != mileageDate){								
							//console.log('value.date = '+value.date);
							//console.log('mileageDate = '+mileageDate);
							var d1 = value.date.split('-');
							var d2 = mileageDate.split('-');						
							//console.log('d1 ' +d1);
							//console.log('d2 '+d2);
							if( parseInt(d1[2]) == parseInt(d2[2])){
								...
								if(flag != 0)
								{
									mileageAmt = parseInt(mileageAmt) + parseInt(value.amt);								
								}
								console.log(mileageAmt);
								car_cost = get_prev_car_miles(mileageDate, mileageAmt);
								console.log(arr[key]['amt']);
								console.log(value.amt);
								//arr[key]['amt'] = car_cost;
								flag = 1;
							}else{	
