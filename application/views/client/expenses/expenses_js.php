...
<script>
    ...
    function openExpenseForm(task, id, title)
    {
        ...
        $.ajax({
            ...
            success: function (msg) {
				...
				var month = $('.MonthDropdown').find('option:selected').val();     
				var year = $('.YearDropdown').find('option:selected').val();
				var date = new Date();			
				var lastDay = new Date(year, month, 0);			
				var newdate = lastDay.getDate()+'-'+(lastDay.getMonth()+1)+'-'+lastDay.getFullYear();
				$('.expense-Date').val(newdate);
            }
        });
    }
    ...
    $(document).ready(function () {
        ...
        $(document).off('click', '.addExpenseItem').on('click', '.addExpenseItem', function (e) {
            var lastRowDate = $('#expenseListItem tr ').find('td:eq(1) input').val();
			e.preventDefault();
			...
			$('#expenseListItem tr:last-child .expense-Date').val(lastRowDate);
...
<?php endif; ?>
	...
	$(document).on('change', '.MonthDropdown', function (e, i) {
		var month = $(this).val();				
		var year = $('.YearDropdown').find('option:selected').val();					
		var date = new Date();			
		var lastDay = new Date(year, month, 0);			
	...
	$(document).on('change', '.YearDropdown', function (e, i) {
