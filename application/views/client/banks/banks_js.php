<script>
    ...
    {       
		...
		$.ajax({
            ...
            success: function (msg) {			
				...
				//$(document).find('#script').find('#customer option:first-child').attr("selected", "selected");
				//$(document).find("#customer option:first").attr('selected','selected');
				//$("#customer").val($("#customer option:first").val());
				$('#modal-statements').on('shown.bs.modal', function (e) { 
					// 1 - select second option, set to 0 for first option
					$("#customer").val($("#customer option:eq(1)").val());
					// $("#customer").val($("#customer option:first").val());
				});
    ...
    $(document).ready(function (e) {
        ...
        $(document).on('click', '.uploadStatement', function (e) {			
			...
			if (b == '' || b == undefined)
            {
                ...
                var text = '<?php echo $this->lang->line('CLIENT_SELECT_BANK_DIALOG_TEXT'); ?>';
        ...
        $(document).on('click', '.createBankInvoice', function (e) {
			var pdate = $('#bankTransactionDate').text();
			$('#modal-linkto').modal('hide');
			//return false;
            e.preventDefault();
        ...
        $(document).on('change', '.category', function (e) {
			...
			$(this).addClass("appendCustomer");			
			if($.trim(catName) == 'addnew'){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo site_url(); ?>clients/Customers/customerFormBank/",
					data: {'id': id, 'page': 'bankstatment'},
					beforeSend: function () {						
						initSpinnerFunction("<?php echo base_url(); ?>assets/loading.gif");
						showSpinner();
					},
            ...
            if (text == 'Dividend'){
				...
				//alert('hiiii sales selected');
                $(this).closest('tr').find('.createBankDividend').addClass('hide');
                $(this).closest('tr').find('.createBankInvoice').removeClass('hide');
				$(this).closest('td').html('<br><input class="form-control" type="text" value="" name="sales" required>');
            } else {
                $(this).closest('tr').find('.createBankDividend').addClass('hide');
                $(this).closest('tr').find('.createBankInvoice').addClass('hide');
            }
    ...
    function validateField(id, error)
    {
        ...
        $(id).each(function (e, v) {
            ...
            if ($(id).val() == '' || $(id).val() == 0)
            {
                ...
                $(id).closest('td').find('select').css('border','1px solid #ff0000');
    ...
    $(document).on('click', '#filterExcel', function (e) {
        e.preventDefault();
        var col = [];
	...
	$(document).on('click', '.linkBankStatmentBtn', function (e) { 		
		var id = $(this).data('id');
		//alert(id);
		if(id == 'All'){
			var category = 'All';	
			$.ajax({
				type: "POST",
				url: "<?php echo site_url();?>getStatmentLinkPage",
				data: {'id':id, category:category},
				beforeSend: function () {  				
				},
				success: function (data) { 
					console.log(data);				
					$('.modal-linkto .modal-title').html('<?php echo $this->lang->line('BANK_STATMENT_LINK'); ?>');
					$('.modal-linkto .modal-body').html(data);
					$('.modal-linkto').modal('show');
				}
			});
		}else{
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url();?>getStatmentLinkPage",
				data: {'id':id, catid:catid,  category:category},
				beforeSend: function () {  				
				},
				success: function (data) { 
					console.log(data);				
	...
	$(document).on('keyup', '.linkbankstatment ', function (e) { 
        //var thisCategory = $(".category").val();
        var searchBy = $(this).val().toUpperCase();
		//alert(searchBy);
        if (searchBy != '') {
            $.ajax({
                type: "post",                
				url: "<?php echo site_url(); ?>searchStatmentLinkItem",
                    ...
                    try {
                        ...
                        if (data != '') {
                            for (var i = 0; i <= data.length; i++) {
                                $('#clientUl').append('<li><a href="javascript:void(0)" class="currentItem" rel="' + data[i].InvoiceNumber + '">' + data[i].InvoiceNumber + '</a></li>');
							}
                        } else {
                            $('#clientUl').append('<li id="recordNot">No Record Found</li>');
                            $('.carOwner').val('');
                        }
    ...
    $(document).on('click', '.currentItem', function () {
        var currentVal = $(this).attr('rel');
        //alert(currentVal);
		$(this).closest('td').find(".linkbankstatment").val(currentVal); 
        $('#clientList').hide('hide');       
    });
	 
	$(document).on('click', '.linkThis', function () {
        var id = $(this).data('id');
		var statmentId = $('#statmentId').val();		
		var name = $(this).data('name');
		if($('.linkedTo').html() == ''){
			$('.linkedTo').append(name);
		}else{
			$('.linkedTo').html(name);
		}
		//Update Bank Statment Associate value 
		$.ajax({
			type: "post",                
			url: "<?php echo site_url(); ?>updateStatementsAssociate",
			//beforeSend: function() { $('#wait').show(); },
			//complete: function() { $('#wait').hide(); },
			cache: false,
			data: {id: id, statmentId: statmentId},
			dataType: "json",
			success: function (data) {
				//console.log(data);
				//alert(data);				
	...
	$('#modal-linkto').on('hidden', function (){
		//alert('sdfsdfds');
		location.reload();
	});
	$(document).on('click', '.nextBankStatmentToLink', function (){
		/*var count = $(".linkCheckbox:checked").length;
		if(count == 0){
			alert('Please select an item first');
			return false;
		}*/
		var page = $('.nextBankStatmentToLink').attr('data-page');
		var total = $(this).data('total');	
		if(page == total){
			//var title = 'Message';
			//var text = 'This is last bank statment to link';
			//dialogBox(title,text);
			alert('This is last bank statment to link');
			return false;
		}
		//Update Bank Statment Associate value 
		$.ajax({
			type: "post",                
			url: "<?php echo site_url(); ?>getAllStatmentForLink",			
			cache: false,
			...
			success: function (data) {
				...
				$('#link-bank-listing').html(data.html);				
				$('.nextBankStatmentToLink').attr("data-page",data.page);  
	...
	$(document).on('change', '.linkCheckbox', function (){		
		$(".linkCheckbox").prop('checked', false);
		$(this).prop('checked', true);
		var id = $(this).data('id');
		var statmentId = $('#statmentId').val();		
		var name = $(this).data('name');		
		$(".closeBankStatmentToLink").attr("data-id",id); 
		$(".closeBankStatmentToLink").attr("data-sid",statmentId);		
		$('.linkedTo').html(name);
		$('#linkaction').show();
		
	});
		
	$(document).on('click', '.closeBankStatmentToLink', function (){
		var id = $(this).data('id');
		var statmentId = $(this).data('sid');
		var statmentType = $('#statmentType').val();
		var statmentAmount = $('#statmentAmount').val();
		//Update Bank Statment Associate value 
		$.ajax({
			type: "post",                
			url: "<?php echo site_url(); ?>updateStatementsAssociate",
			...
			//complete: function() { $('#wait').hide(); },
			cache: false,
			data: {id: id, statmentId: statmentId, statmentType: statmentType, statmentAmount: statmentAmount},
			dataType: "json",
			success: function (data){
				console.log(data);
				$('#modal-linkto').modal("hide");
	...
	$(document).on('click', '#create-customers', function (e) {	
		...
		$.ajax({
			...
			success: function (msg) {
				hideSpinner();
				console.log(msg);
				$('.appendCustomer').append($("<option selected></option>").attr('value',msg.id).text(msg.name));									
				//$("#appendCustomer").append("<option value="+msg.id+" selected>"+msg.name+"</option>");							
				//$(".appendCustomer").append("<option value='31'>xcvxcv</option>");					
