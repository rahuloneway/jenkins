...
<?php 
	...
	$pageController= $this->uri->segment(1);	
	//echo "<pre>"; print_r($menus); echo "</pre>";
	
	function checkInSubMenu($array, $key, $val) {
	...
	//$array = $menus[1]['subMenus'];
	//$key = "url";
	//$val = "invoices";
	
	//echo checkInSubMenu($array,$key,$val); die('dsfsd');
?>
<!DOCTYPE html>
<html lang="en">
<head>
	...
	<script type="text/javascript" src="<?php echo site_url();?>assets/js/jquery.maskedinput.min.js"></script>
      ...
      <div class="col-md-12">
		...
		<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
			<ul class="nav navbar-nav">
				<li class="li_bg <?php if($pageController == 'client') { echo "active";}?>"><a href="<?php echo site_url('client');?>">Dashboard</a></li>
				
				<?php 
				if( !empty($menus)){
					foreach( $menus as $menu){ //echo "###".checkInSubMenu($menu['subMenus'],'url',$pageController); ?>						
						<li class="li_bg <?php if($pageController == $menu['url']) { echo "active";}?> <?php if( !empty($menu['subMenus'])){ ?>dropdown<?php } ?>  <?php if(checkInSubMenu($menu['subMenus'],'url',$pageController) == 1){ echo "open active"; } ?>">
							...
							<?php	foreach($menu['subMenus'] as $subMenu){ ?>
								<li class="<?php if($pageController == $subMenu['url']) { echo "active";}?> ">
