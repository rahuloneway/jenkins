...
class Lioptws extends CI_Controller {
        ...
        */
		...
		if ($userData) {
			...
			if ($response == 'id_error') {				
				$error = "There is no account associated. Please check your details.";
			} elseif ($response == 'password_error') {				
				$error = "There was an error with your Username or Password combination. Please check your details.";			
			} else {				
				$error = "There is no account associated. Please check your details.";				
			}			
		}       
    ...
    */ 
    private function _login_user($username, &$error) {
        ...
        */     
        $this->load->model('login');
		$userData = $this->login->isPortalUserExist($username, $password);		
    ...
    */ 
    public function TryLogin($username = NULL, $password = NULL) {
        ...
        */
        ...
        if($_result){ //valid user
            ...
            $_responce = array(
                ...
                'data' => array(
                    'link' => str_replace('@','%40',$_loginLink),
                )
            ); 
    ...
    */ 
    public function Login($username, $key) {
        ...
        //$username, $key, we need to get this from GET
