    ...
    public function sendMail() {	
		...
		if (empty($bulkEmailUserDetails)) { 
			...
			foreach ($bulkEmailUserDetails as $val) {	
				echo "<pre>"; print_r($user); print_r($val); echo "</pre>";
				if(isset($val[2])){
					#Get querter dwtails
				...
				if (!empty($val)) {
					...
					if (!empty($clientinfo[0]->Email)) {
						...
						if (strpos($msg, '{ACCEPT}') !== false) {
							$acceptLink = $clientinfo[0]->CID."-".$thisQuater.'-accept';
						...
						if (strpos($msg, '{REJECT}') !== false) {
							$rejectLink = $clientinfo[0]->CID."-".$thisQuater.'-rejects';
