...
if (!function_exists('userDasboard')) {
    ...
    */
    ...
    function getCutomername($id) {
        $ci = & get_instance();
        $prefix = $ci->db->dbprefix;
        $query = $ci->db->query('SELECT companyname FROM ' . $prefix . 'customers WHERE ID=' . $id);
		//echo $ci->db->last_query();
        if ($query->num_rows() > 0) {
            $record = $query->result();
            $record = $record[0]->companyname;
            return $record;
        } else {
            return FALSE;
        }
	...
	function getCutomerNameByTBCat($tbCat) {
        $ci = & get_instance();
        $prefix = $ci->db->dbprefix;
        $query = $ci->db->query('SELECT companyname FROM ' . $prefix . 'customers WHERE catKey=' . $tbCat);
		//echo $ci->db->last_query();
        if ($query->num_rows() > 0) {
            $record = $query->result();
            $record = $record[0]->companyname;
	...
	function getAllCustomer($clientId) {
        $ci = & get_instance();
        $prefix = $ci->db->dbprefix;
        $query = $ci->db->query('SELECT first_name FROM ' . $prefix . 'customers WHERE clientId=' . $clientId);
		//echo $ci->db->last_query();
        if ($query->num_rows() > 0) {
            $record = $query->result();
            $record = $record;
	...
	function getAllSupplier($clientId) {
        $ci = & get_instance();
        $prefix = $ci->db->dbprefix;
        $query = $ci->db->query('SELECT first_name FROM ' . $prefix . 'suppliers WHERE clientId=' . $clientId);
		//echo $ci->db->last_query();
        if ($query->num_rows() > 0) {
            $record = $query->result();           
            return $record;
	...
	#Get Parent Name By Category ID
	function getCategoryParentName($id){  
		$ci = &get_instance();
        $prefix = $ci->db->dbprefix;        
		$ci->db->select('AnalysisLedgerParent');
		$query = $ci->db->get_where('cashman_trial_balance_categories', array('id' => $id));
		$result = $query->result();	
		//echo $ci->db->last_query();		
		if($result[0]->AnalysisLedgerParent != ''){
			$ci->db->select('title');
			$query2 = $ci->db->get_where('cashman_trial_balance_categories', array('id' => $result[0]->AnalysisLedgerParent));
			$result2 = $query2->result();
			//echo $ci->db->last_query();
			return $result2[0]->title;
		}else{
			return "";
		}		
	}
	...
	function getCategoryParentId($id){  
		...
		$result = $query->result();			
		echo $ci->db->last_query();	
		return $result[0]->AnalysisLedgerParent;
	}
	#Get Parent Name By Category ID
	function getCategoryParentNameByKey($key){  
		$ci = &get_instance();
        $prefix = $ci->db->dbprefix;
        
		$ci->db->select('parent');
		$query = $ci->db->get_where('cashman_trial_balance_categories', array('catKey' => $key));
		$result = $query->result();						
		if($result[0]->parent != ''){
			$ci->db->select('title');
			$query2 = $ci->db->get_where('cashman_trial_balance_categories', array('id' => $result[0]->parent));
			$result2 = $query2->result();
			return $result2[0]->title;
		}else{
			return "";
	...
	function getCategoryParentChild($parentId){	 
		...
		if($parentId == 235){ #customer child categry			
			...
			if ($query->num_rows() > 0) {
				$i=0;
				foreach($result as $key => $val) { 
					$ci->db->select('id,CONCAT(FirstName,' . ',LastName) AS title');			
					//$ci->db->order_by('id', 'asc');
					$query2 = $ci->db->get_where('cashman_company_customers', array('TbCategoryshareholder =' => $val->catKey,'CompanyID'=>$user['CompanyID']));
					$result2 = $query2->result();
					//echo $ci->db->last_query(); 
					if ($query2->num_rows() > 0) {
						$result2[0]->id = $val->id; 
			...
			if ($query->num_rows() > 0) {
				...
				foreach($result as $key => $val) { 
					$ci->db->select('id,CONCAT(FirstName,' .', LastName) AS title');			
					//$ci->db->order_by('id', 'asc');
					$query2 = $ci->db->get_where('cashman_company_customers', array('TbCategoryEmployee =' => $val->catKey,'CompanyID'=>$user['CompanyID']));
					$result2 = $query2->result();
					//echo $ci->db->last_query(); //die;
					if($query2->num_rows() > 0) {
						$result2[0]->id = $val->id; 
	...
	function getCategoryKey($id){  
		...
		$result = $query->result();						
		if(!empty($result)){		
			return $result[0]->catKey;
		}else{
			return "";
		}		
	}	
	
	#Get Category Vat Type Using Category ID
	function getCategoryVatType($id){  
		$ci = &get_instance();
        $prefix = $ci->db->dbprefix;        
		$ci->db->select('VatType');
		$query = $ci->db->get_where('cashman_trial_balance_categories', array('id' => $id));
		//echo $ci->db->last_query();
		$result = $query->result();						
		if(!empty($result)){		
			return $result[0]->VatType;
		}else{
			return "";
		}		
	}	
	...
	function getCategoryType($title){  
		$ci = &get_instance();
        $prefix = $ci->db->dbprefix;        
		$ci->db->select('CategoryType');
		$query = $ci->db->get_where('cashman_trial_balance_categories', array('title' => $title));
		//echo $ci->db->last_query();
		$result = $query->result();						
		if(!empty($result)){		
			return $result[0]->CategoryType;
		}else{
			return "";
		}		
	}
	...
	function getCategoryName($id){		
		...
		$user = $ci->session->userdata('user');			
		$ci->db->select('CategoryType');
		$queryType = $ci->db->get_where('cashman_trial_balance_categories', array('id' => $id));
		//echo $ci->db->last_query();
		$resultType = $queryType->result();						
		if(!empty($resultType)){		
			if($resultType[0]->CategoryType == 'CUSTOMERS'){
				$ci = &get_instance();
				$prefix = $ci->db->dbprefix;        
				$ci->db->select('catKey');
				$queryCus = $ci->db->get_where('cashman_trial_balance_categories', array('id' => $id));
				//echo $ci->db->last_query();
				$resultCus = $queryCus->result();	
				if(!empty($resultCus)){	
					$ci->db->select('id,companyname AS title');			
					$query = $ci->db->get_where('cashman_customers', array('TB_Category =' => $resultCus[0]->catKey,'clientCID'=>$user['UserID']));
					$result = $query->result();					
					return $result[0]->title;				
				}
			}else if($resultType[0]->CategoryType == 'SUPPLIERS'){
				$ci = &get_instance();
				$prefix = $ci->db->dbprefix;        
				$ci->db->select('catKey');
				$querySup = $ci->db->get_where('cashman_trial_balance_categories', array('id' => $id));
				//echo $ci->db->last_query();
				$resultSup = $querySup->result();	
				if(!empty($resultSupp)){	
					$ci->db->select('id, CONCAT(first_name, '.', last_name) AS title');			
					$query = $ci->db->get_where('cashman_suppliers', array('TB_Category =' => $resultSup[0]->catKey,'ClientID'=>$user['UserID']));
					echo $ci->db->last_query(); 
					$result = $quer->result();					
					return $result[0]->title;				
				}
			}else if($resultType[0]->CategoryType == 'SHAREHOLDERS' ){
				$ci = &get_instance();
				$prefix = $ci->db->dbprefix;        
				$ci->db->select('catKey');
				$queryShare = $ci->db->get_where('cashman_trial_balance_categories', array('id' => $id));
				//echo $ci->db->last_query(); 
				$resultShare = $queryShare->result();	
				if(!empty($resultShare)){	
					$ci->db->select('id,CONCAT(FirstName, '.', LastName) AS title');			
					$query = $ci->db->get_where('cashman_company_customers', array('TbCategoryshareholder =' => $resultShare[0]->catKey,'CompanyID'=>$user['CompanyID']));					
					$result = $query->result();					
					return $result[0]->title;				
				}
			}else if($resultType[0]->CategoryType == 'EMPLOYEES' || $resultType[0]->CategoryType == 'BOTH'){
				$ci = &get_instance();
				$prefix = $ci->db->dbprefix;        
				$ci->db->select('catKey');
				$querySup = $ci->db->get_where('cashman_trial_balance_categories', array('id' => $id));
				//echo $ci->db->last_query();
				$resultEmp = $querySup->result();	
				if(!empty($resultEmp)){	
					$ci->db->select('ID,CONCAT(FirstName, '. ', LastName) AS title');			
					//$query = $ci->db->get_where('cashman_company_customers', array('TB_Category =' => $resultSup[0]->catKey,'CompanyID'=>$user['CompanyID'],'IS_Employee'=>1));
					$query = $ci->db->get_where('cashman_company_customers', array('CompanyID'=>$user['CompanyID'],'IS_Employee'=>1,'CompanyID'=>$user['CompanyID']));
					//echo "#".$ci->db->last_query();
					$result = $query->result();						
					return $result[0]->title;				
				}
		...
		if($id != 235){
			...
			if ($query->num_rows() > 0) {
				...
				foreach ($result as $key => $val) { 
					$ci->db->select('id,CONCAT(first_name, '.', last_name) AS title');			
					//$ci->db->order_by('id', 'asc');
					$query2 = $ci->db->get_where('cashman_customers', array('TB_Category =' => $val->catKey));
					$result2 = $query2->result();
					$result2[0]->id = $val->id; 
					$customerName = $result2[0];					
					$i++;
				}
			} else {
				$customerList = '';
	...
	#Get All Customer Categories from trial_balance_categories table
	function getAllCustomerCategory(){  
		$ci = &get_instance();
        $prefix = $ci->db->dbprefix;        
		$ci->db->select('title');
		$query = $ci->db->get_where('cashman_trial_balance_categories', array('AnalysisLedgerParent' => 235));
		//echo $ci->db->last_query();
		$result = $query->result();						
		if(!empty($result)){		
			return $result;
	...
	#Get All Supplier Categories from trial_balance_categories table
	function getAllSupplierCategory(){  
		$ci = &get_instance();
        $prefix = $ci->db->dbprefix;        
		$ci->db->select('title');
		$query = $ci->db->get_where('cashman_trial_balance_categories', array('AnalysisLedgerParent' => 242));
		//echo $ci->db->last_query();
		$result = $query->result();						
		if(!empty($result)){		
			return $result;
	...
	#Get All Shareholder Categories from trial_balance_categories table
	function getAllShareholderCategory(){  
		$ci = &get_instance();
        $prefix = $ci->db->dbprefix;        
		$ci->db->select('title');
		$query = $ci->db->get_where('cashman_trial_balance_categories', array('AnalysisLedgerParent' => 279, 'catKey LIKE' => 'SHAREHOLDER%'));
		//echo $ci->db->last_query();
		$result = $query->result();						
		if(!empty($result)){		
			return $result;
	...
	#Get All Employee Categories from trial_balance_categories table
	function getAllemployeesCategory(){  
		$ci = &get_instance();
        $prefix = $ci->db->dbprefix;        
		$ci->db->select('title');
		$query = $ci->db->get_where('cashman_trial_balance_categories', array('AnalysisLedgerParent' => 279, 'catKey LIKE' => 'EMPLOYEE%'));
		//echo $ci->db->last_query();
		$result = $query->result();						
		if(!empty($result)){		
			return $result;
	...
	#Get All Both Shareholder And Employee Categories from trial_balance_categories table
	function getAllBothCategory(){  
		$ci = &get_instance();
        $prefix = $ci->db->dbprefix;        
		$ci->db->select('title');
		$query = $ci->db->get_where('cashman_trial_balance_categories', array('AnalysisLedgerParent' => 279, 'catKey LIKE' => 'BOTH%'));
		//echo $ci->db->last_query();
		$result = $query->result();						
		if(!empty($result)){		
			return $result;
...
// 		$query = $ci->db->get($prefix.'company');
    ...
    function itemNumber($id = null, $type = null) {
        if (!empty($id) && !empty($type)) {
            ...
            if ($type == 'I') {
                ...
                $ci->db->select('InvoiceNumber');
                $response = $ci->db->get_where('purchases', array('InvoiceID' => $id));
                $response = $response->result();
                $response = $response[0]->InvoiceNumber;
                return $response;
            }elseif ($type == 'D') {
                ...
                $ci->db->select('ExpenseNumber');
                $response = $ci->db->get_where('expenses', array('ID' => $id));
                $response = $response->result();
                $response = $response[0]->ExpenseNumber;
                return $response;
            }
        } else {
            return '';
    ...
    function calculateVatQuarters($clientId = 0) {
        if ($clientId > 0) {
			...
			foreach ( $response->result() as $row ) {
                ...
                $result = $ci->db->get_where('vat_quarters', array('ClientID' => $clientId, 'companyID' => $CID));
                $vatQua = $result->row();
                if (count($vatQua) > 0) {
    ...
    function vatYearDropDown($name = 'vatYear', $option = NULL, $selected = 0, $id = NULL) {
        ...
        echo "###".$VATYear = $ci->session->userdata('VATYear'); 
        if (!empty($VATYear)) {
            $selected = $VATYear;
        } else if ($selected == 0) {
            $selected = date("Y");
     ...
     */
    function getVatQuarters($vYear = NULL, $clientId = NULL) {
		...
		//echo "==========>".$ci->db->last_query();
        if ($row = $result->row()) {
			/*if($row->q1_start == 11 || $row->q1_start == 12){
				$vYear = $vYear - 1;
			}*/			
        ...
        //echo "==============<pre>"; print_r( $vatQuaters );echo "</pre>";//die('globle healper 1810');
        return $vatQuaters;
    }
    ...
    */
        ...
        if (!empty($type)) {
            ...
            switch ($type) {
                    ...
                    if (!empty($invData)) {
                        ...
                        if ($from != "BANK" && $from != "BANK_DEL") {
							...
							if(isset($invData["CIS_Deduction"]) && $invData["CIS_Deduction"] != 0){
								$amount = $invData["InvoiceTotal"]-$invData["CIS_Deduction"];
							}else{
								$amount = $invData["InvoiceTotal"];
							}
                        }
                        ...
                        if (!empty($from)) {
                            ...
                            if ($action == "DELETE") {
                             ...
                             //   if ((int) $TBentryId > 0) {
                                    add_trial_details($TBCatId, $invData["PaidOn"], $tbSource, "SALES", $aAccess, $invData["id"], $invData["InvoiceSub"], $paidstatus, "SUBTRACT");
                              //  }
                            }
							...
							#Get customer trial balance customer category from customer table							
							$TBCustomerCat = getTBCustomerCategory($invData["UserID"]);
							$TBCatId = get_trial_balance_category($TBCustomerCat); //get category ID for given key
                            
							if ($action == "DELETE") {
                              //  $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $invData["InvoiceTotal"], "SUBTRACT");
                              //  if ((int) $TBentryId > 0) {
                                    rm_trial_details($tbSource, $TBCustomerCat, $invData["id"]);
                              //  }
                            } else {
                              //  $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $invData["InvoiceTotal"]);
                               // if ((int) $TBentryId > 0) {
                                    add_trial_details($TBCatId, $invData["PaidOn"], $tbSource, $TBCustomerCat, $aAccess, $invData["id"], $amount ,$paidstatus);
                              //  }
                            }

                            // VAT Control Suspense goes up By "Flat/Standard Amount"
                            $TBCatId = get_trial_balance_category("VAT - Deferred output");
                            if ($action == "DELETE") {
                              //  $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $govtVAT);
                              //  if ((int) $TBentryId > 0) {
                                    rm_trial_details($tbSource, "VAT - Deferred output", $invData["id"]);
                              //  }
                            } else {
                              //  $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $govtVAT, "SUBTRACT");
                              //  if ((int) $TBentryId > 0) {
                                    add_trial_details($TBCatId, $invData["PaidOn"], $tbSource, "VAT - Deferred output", $aAccess, $invData["id"], $govtVAT, $paidstatus, "SUBTRACT");
									
							  //  }
                            }
							
							
							// Other income TB entry if Vat type is Flat"
							if($invData['VatType'] == 'flat'){
								$TBCatId = get_trial_balance_category("OTHER_OPERATING_INCOME");
								if ($action == "DELETE") {
								  //  $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $govtVAT);
								  //  if ((int) $TBentryId > 0) {
										rm_trial_details($tbSource, "OTHER_OPERATING_INCOME", $invData["id"]);
								  //  }
								} else {
								  //  $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $govtVAT, "SUBTRACT");
								  //  if ((int) $TBentryId > 0) {
										add_trial_details($TBCatId, $invData["PaidOn"], $tbSource, "OTHER_OPERATING_INCOME", $aAccess, $invData["id"], ($invData["InvoiceTax"]-$govtVAT), $paidstatus, "SUBTRACT");
							...
							if(isset($invData["CIS_Deduction"]) && $invData["CIS_Deduction"] != 0){
								...
								if ($action == "DELETE") {
								  ...
								  //  $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $govtVAT, "SUBTRACT");
								  //  if ((int) $TBentryId > 0) {
										add_trial_details($TBCatId, $invData["PaidOn"], $tbSource, "CIS_DEBTOR", $aAccess, $invData["id"], $invData["CIS_Deduction"], $paidstatus);
								  //  }
								}
							}
                ...
                case "salary":
                    ...
                    if (!empty($salData)) {
                        if (!empty($from)) {
                            ...
                            //$TBCatId = get_trial_balance_category("SALARY_SUSPENSE"); //get category ID for given key
                            #Get customer trial balance customer category from customer table							
							$TBEmployeeCat = getTBEmployeeCategory($salData["EID"]);
							$TBCatId = get_trial_balance_category($TBEmployeeCat); //get category ID for given key
							
							if ($action == "DELETE") {
                             //   $TBentryId = store_trial_entry($TBCatId, $salData["PaidDate"], $salData["NetPay"]);
                             //   if ((int) $TBentryId > 0) {
                                    rm_trial_details($tbSource, $TBEmployeeCat, $salData["id"]);
                             //   }
                            } else {
                             //   $TBentryId = store_trial_entry($TBCatId, $salData["PaidDate"], $salData["NetPay"], "SUBTRACT");
                             //   if ((int) $TBentryId > 0) {
                                    add_trial_details($TBCatId, $salData["PaidDate"], $tbSource, $TBEmployeeCat, $aAccess, $salData["id"], ($salData["GrossSalary"]+$salData["Employeer_NIC"]),$paidstatus, "SUBTRACT");
                             //   }
                            }
				...
				case "purchase":
                    ...
                    if (!empty($invData)) {
                        ...
                        if (!empty($from)) {							
							...
							// Sales goes up By "Sales Amount" Minus "VAT Paid to Govt."
                            //$TBCatId = get_trial_balance_category("SALES"); //get category ID for given key
							$TBCatCatKey = getCategoryKey($invData["Category"]);													
							$TBCatId = $invData["Category"];
                            ...
                            if ($action == "DELETE") {
                              ...
                              //  $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $amount, "SUBTRACT");
                             //   if ((int) $TBentryId > 0) {
                                    add_trial_details($TBCatId, $invData["PaidOn"], $tbSource, $TBCatCatKey, $aAccess, $invData["id"], $invData["InvoiceSub"], $paidstatus);
                              //  }
                            }

                            // Sales Suspenses goes up By "Sales Amount"
							$TBSupplierCat = getTBSupplierCategory($invData["UserID"]);
                            $TBCatId = get_trial_balance_category($TBSupplierCat); //get category ID for given key
							if ($action == "DELETE") {
                              //  $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $invData["InvoiceTotal"], "SUBTRACT");
                              //  if ((int) $TBentryId > 0) {
                                    rm_trial_details($tbSource, $TBSupplierCat, $invData["id"]);
                              //  }
                            } else {
                              //  $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $invData["InvoiceTotal"]);
                               // if ((int) $TBentryId > 0) {
                                    add_trial_details($TBCatId, $invData["PaidOn"], $tbSource, $TBSupplierCat, $aAccess, $invData["id"], $invData["InvoiceTotal"],$paidstatus, "SUBTRACT");
                              //  }
                            }

                            // VAT Control Suspense goes up By "Flat/Standard Amount"
                            $TBCatId = get_trial_balance_category("VAT - Deferred input");
                            if ($action == "DELETE") {
                              //  $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $govtVAT);
                              //  if ((int) $TBentryId > 0) {
                                    rm_trial_details($tbSource, "VAT - Deferred input", $invData["id"]);
                              //  }
                            } else {
                              //  $TBentryId = store_trial_entry($TBCatId, $invData["PaidOn"], $govtVAT, "SUBTRACT");
                              //  if ((int) $TBentryId > 0) {
                                    add_trial_details($TBCatId, $invData["PaidOn"], $tbSource, "VAT - Deferred input", $aAccess, $invData["id"], $invData["InvoiceTax"], $paidstatus);
                ...
                case "expense":
                    ...
                    // prd($data);
                    //echo "<pre>";print_r($expData); echo "</pre>";
                        ...
                        if (!empty($from)) {
                            ...
                            if (isset($expData["items"]) && !empty($expData["items"])) {
                                foreach ($expData["items"] as $expItem) {									
									...
									if($expItem["ItemType"] == 'MILEAGE'){
										$TBCatCatKey = 'TRAVEL_AND_SUBSISTENCE';
										$TBCatId = 22;
										 $expAmount = $expItem["Amount"];
									}else{
										$TBCatCatKey = getCategoryKey($expItem["Category"]);
										$TBCatId = $expItem["Category"];
										//$expAmount = $expItem["Amount"]-$expItem["VATAmount"];
										$expAmount = $expItem["Amount"];
									}	
                                    ...
                                    if ($TBCatId) {
                                        ...
                                        if ($action == "DELETE") {
                                          ...
                                          //  if ((int) $TBentryId > 0) {
                                                rm_trial_details($tbSource, $TBCatCatKey, $expItem["id"]);
                                          //  }
                                        } else {
                                          //  $TBentryId = store_trial_entry($TBCatId, $expData["PaidOn"], $expAmount);
                                          //  if ((int) $TBentryId > 0) {
                                                add_trial_details($TBCatId, $expData["PaidOn"], $tbSource, $TBCatCatKey, $aAccess, $expItem["id"], $expAmount,$paidstatus);
                                          //  }
                                        }
                                    }
                                    ...
                                    if ($expData["ExpenseType"] == "CREDITCARD") {
                                        ...
                                        $TBEmployeeCat = getTBEmployeeCategory($expData["EmployeeID"]);
										$TBCatId = get_trial_balance_category($TBEmployeeCat);
                                    ...
                                    if ($action == "DELETE") {
                                      ...
                                      //  $TBentryId = store_trial_entry($TBCatId, $expData["PaidOn"], $expAmount, "SUBTRACT");
                                      //  if ((int) $TBentryId > 0) {
                                            add_trial_details($TBCatId, $expData["PaidOn"], $tbSource, $tbDetailType, $aAccess, $expItem["id"], ($expItem["Amount"]+$expItem["VATAmount"]),$paidstatus, "SUBTRACT");
                                      //  }
                                    }
                                }
                            }
                            ...
                            //if ($standardClient) {
                                ...
                                if ($action == "DELETE") {
                                 ...
                                 //   if ((int) $TBentryId > 0) {
                                        add_trial_details($TBCatId, $expData["PaidOn"], $tbSource, "VAT - Deferred input", $aAccess, $expData["id"], $expData["TotalVATAmount"],$paidstatus);
                                 //   }
                                }
                            //}
                ...
                case "dividend":
                    ...
                    if (!empty($divdata)) {
                        ...
                        if (!empty($from)) {
                            ...
                            // "Dividend suspense" goes up
							$TBShareholderCat = getTBShareholderCategory($divdata["ShareholderID"]);
                            $TBCatId = get_trial_balance_category($TBShareholderCat); //get category ID for given key
                            if ($action == "DELETE") {
                              //  $TBentryId = store_trial_entry($TBCatId, $divdata["PaidDate"], $divdata["NetAmount"]);
                              //  if ((int) $TBentryId > 0) {
                                    rm_trial_details($tbSource, $TBShareholderCat, $divdata["id"]);
                              //  }
                            } else {
                             //   $TBentryId = store_trial_entry($TBCatId, $divdata["PaidDate"], $divdata["NetAmount"], "SUBTRACT");
                              //  if ((int) $TBentryId > 0) {
                                    add_trial_details($TBCatId, $divdata["PaidDate"], $tbSource, $TBShareholderCat, $aAccess, $divdata["id"], $divdata["NetAmount"],$paidstatus, "SUBTRACT");
                             //   }
                            }
                            return;
                        }
	...
	function pl_categories() {
        $ci = & get_instance();
        $prefix = $ci->db->dbprefix;
        $query = "SELECT id,title,catKey,AnalysisLedgerParent FROM " . $prefix . "trial_balance_categories where AnalysisLedgerParent = 0";
        $query = $ci->db->query($query);       
		$parentCategories = $query->result();		
        $parent = array('0' => '--Select category--');
		$temp = array();
		$user = $ci->session->userdata('user');
		foreach ($parentCategories as $pval) {			
			$parentId = $pval->id;
			if($parentId == 127){ #Cank name					
				$ci->db->select('id,catKey,AnalysisLedgerParent');			
				$ci->db->order_by("catKey", "asc");
				$query = $ci->db->get_where('cashman_trial_balance_categories', array('AnalysisLedgerParent =' => $parentId));
				$result = $query->result();
				//echo $ci->db->last_query(); //die;
				...
				if ($query->num_rows() > 0) {
					$i=0;
					foreach($result as $key => $val) { 								
						$ci->db->select('BID as id,Name as title');							
						$query2 = $ci->db->get_where('cashman_banks', array('TB_Category =' => $val->catKey,'CompanyID'=>$user['CompanyID'],'status'=>1));
						//echo $ci->db->last_query(); //die;
						$childCategories = $query2->result();						
						if ($pval->AnalysisLedgerParent == 0) {
							foreach ($childCategories as $k => $v) {  
								if ($val->AnalysisLedgerParent != 0 && $pval->id == $val->AnalysisLedgerParent) {
									$temp[$v->id] = $v->title;
								}
							}
						}						
						if(!empty($temp)){
							$parent[$pval->title] = $temp;
							unset($temp);
						}
					}
				}		
				
			}elseif($parentId == 235){ #customer child categry					
				$ci->db->select('id,catKey,AnalysisLedgerParent');			
				$ci->db->order_by("catKey", "asc");
				$query = $ci->db->get_where('cashman_trial_balance_categories', array('AnalysisLedgerParent =' => $parentId));
				$result = $query->result();
				...
				if ($query->num_rows() > 0) {
					...
					foreach($result as $key => $val) { 
						...
						$ci->db->select('companyname,id,CONCAT(first_name,' .', last_name) AS title');			
						//$ci->db->order_by('id', 'asc');
						$query2 = $ci->db->get_where('cashman_suppliers', array('TB_Category =' => $val->catKey,'ClientID'=>$user['UserID']));
						$childCategories = $query2->result();						
						//echo $ci->db->last_query(); //die;						
			...
			if ($query->num_rows() > 0) {
				...
				if ($pval->AnalysisLedgerParent == 0) {
					foreach ($childCategories as $k => $v) {
						if ($v->AnalysisLedgerParent != 0 && $pval->id == $v->AnalysisLedgerParent) {
							$temp[$v->id] = $v->title;
						}
					}
				}
        ...
        /*foreach ($categories as $key => $val) {
            if ($val->AnalysisLedgerParent == 0) {
                foreach ($categories as $k => $v) {
                    if ($v->AnalysisLedgerParent != 0 && $val->id == $v->AnalysisLedgerParent) {
                        if($v->title != '')
							$temp[$v->id] = $v->title;
                    }
                }
    ...
    function getPaidYear($date = "") {
        ...
        /*print_r( $nxtDates ); 
		echo date("Y-m-d", $nxtDates['start']);
		echo date("Y-m-d", $nxtDates['end']);		
		die('Globle function 3937');*/
        if ($nxtDates["start"] <= $Time && $Time <= $nxtDates["end"]) {            
        } else {            
            $nxtDates = getTBPaidStartEndDate(strtotime($date), $year );
        }
	...
	#Get invoice FRS Presentage For Revert 
	function getFRS_PresentageForRevert($id){
		$ci = & get_instance();
        $ci->db->select('InvoiceDate,VatType,PercentRateAfterEndDate,PercentRate');
        $query = $ci->db->get_where('invoices', array('InvoiceID' => $id));
        $db_error = $ci->db->error();
        if ($db_error['code'] != 0) {
            log_message('error', $db_error['message']);
        ...
        if ($query->num_rows() > 0) {
			...
			if(strtotime($user['EndDate']) > strtotime($result[0]['InvoiceDate'])){
				$result = $result[0]['PercentRateAfterEndDate'];
			}else{
				$result = $result[0]['PercentRate'];				
			}
	...
	// Remove details from "Trial balance"
    function revert_vat_trial_details($itemId, $SId, $sType, $sAmount) {
        
		#Get existing vat record from trial balance table.
		$ci = & get_instance();
        $prefix = $ci->db->dbprefix;		
			
		if($sType == "Customer"){
			$user = $ci->session->userdata('user');
			$totalPresentage = 120 - $user['Params']['cis_percentage'];
			//$totalPresentage = 120;
			$amount = ($sAmount*100)/$totalPresentage;
			$amount = round($amount,2);
			if($user['VAT_TYPE'] == 'flat'){
				$p = getFRS_PresentageForRevert($itemId);
				$revertVat = ($sAmount*$p)/100;				
			}else{
				$revertVat = ($sAmount*20)/100;
			}			
			$revertVat = round($revertVat,2);
				
			$source = "INVOICE";
			$category_id = 271;
			$revert_category_id = 274;
			$revert_type = "VAT - Output";
			$revertVatValue = -$revertVat;
		}else{
			$totalPresentage = 120;
			
			$amount = ($sAmount*100)/$totalPresentage;
			$amount = round($amount,2);
			$revertVat = ($amount*20)/100;
			$revertVat = round($revertVat,2);
			
			$source = "PURCHASE";
			$category_id = 270;
			$revert_category_id = 273;
			$revert_type = "VAT - Input";
			$revertVatValue = $revertVat;
		}
        
		
        $ci->db->select('*');
        $query = $ci->db->get_where('tb_details', array('itemId' => $itemId, 'source'=>$source, 'category_id'=>$category_id ));
        $result = $query->result();	
		//echo $ci->db->last_query();
		...
		if ($row = $query->row()) {
			$insertData = array(
								...
								'itemStatus' => $result[0]->itemStatus,
								'itemId' => $result[0]->itemId,
								'source' =>	$result[0]->source,
								'type' => $revert_type,
								'amount' =>	$revertVatValue,
								'category_id' => $revert_category_id,
								'addedOn' => $result[0]->addedOn,
								'AccountantAccess' => $result[0]->AccountantAccess,
								'deleteStatus' => $result[0]->deleteStatus,							
							);
			$ci->db->insert('tb_details', $insertData);
			...
			if($sType == "Customer"){
				$updateData = array(
					'amount' => ($result[0]->amount+$revertVat)
				);
			}else{
				$updateData = array(
					'amount' => ($result[0]->amount-$revertVat)
			...
			$ci->db->where($where);
			$ci->db->update('tb_details', $updateData);			
	...
	#GET Customer Category from customer table by customer id
	function getTBCustomerCategory($id) {
		$ci = &get_instance();
		$prefix = $ci->db->dbprefix;
		$ci->db->select('TB_Category');			
		$query = $ci->db->get_where('cashman_customers', array('id' => $id));
		//echo $ci->db->last_query(); //die;
		$result = $query->result();					
		if(!empty($result)){
			return $result[0]->TB_Category;
		} else {
			return '';
		} 		
	}
	
	#GET Supplier Category from customer table by customer id
	function getTBSupplierCategory($id) {
		$ci = &get_instance();
		$prefix = $ci->db->dbprefix;
		$ci->db->select('TB_Category');			
		$query = $ci->db->get_where('cashman_suppliers', array('id' => $id));
		//echo $ci->db->last_query(); //die;
		$result = $query->result();					
		if(!empty($result)){
			return $result[0]->TB_Category;
	...
	#GET Shareholder Category from customer table by customer id
	function getTBShareholderCategory($id) {		
		$ci = &get_instance();
		$prefix = $ci->db->dbprefix;
		$ci->db->select('TbCategoryshareholder');			
		$query = $ci->db->get_where('cashman_company_customers', array('ID' => $id));
		//echo $ci->db->last_query(); //die;
		$result = $query->result();					
		if(!empty($result)){
			return $result[0]->TbCategoryshareholder;
		} else {
			return '';
		} 		
	}
	
	#GET Employee Category from customer table by customer id
	function getTBEmployeeCategory($id) {
		$ci = &get_instance();
		$prefix = $ci->db->dbprefix;
		$ci->db->select('TbCategoryEmployee');			
		$query = $ci->db->get_where('cashman_company_customers', array('id' => $id, 'IS_Employee' => 1));
		//echo $ci->db->last_query(); //die;
		$result = $query->result();					
		if(!empty($result)){
			return $result[0]->TbCategoryEmployee;
		} else {
			return '';
		} 		
	} 
	...
	#GET Customer Name 
	function getCustomerNameTBListing($title) {		
		$ci = &get_instance();
		$user = $ci->session->userdata('user');
		$prefix = $ci->db->dbprefix;
		$ci->db->select('catKey');			
		...
		if(!empty($result)){
			...
			$query2 = $ci->db->get_where('cashman_customers', array('TB_Category' => $result[0]->catKey, 'clientId' => $user['UserID']));
			//echo $ci->db->last_query();
			$result2 = $query2->result();			
			return $result2[0]->first_name;			
		} else {
			return '';
		} 		
	}
	#GET Supplier Name 
	function getSupplierNameTBListing($title) {		
		$ci = &get_instance();
		$user = $ci->session->userdata('user');
		$prefix = $ci->db->dbprefix;
		$ci->db->select('catKey');			
		...
		if(!empty($result)){
			$ci->db->select('first_name');
			$query2 = $ci->db->get_where('cashman_suppliers', array('TB_Category' => $result[0]->catKey, 'clientId' => $user['UserID']));
			$result2 = $query2->result();			
			return $result2[0]->first_name;			
		} else {
			return '';
	...
	function getShareholderNameTBListing($title) {		
		$ci = &get_instance();
		$user = $ci->session->userdata('user');
		$prefix = $ci->db->dbprefix;
		$ci->db->select('catKey');			
		...
		if(!empty($result)){
			...
			$query2 = $ci->db->get_where('cashman_company_customers', array('TbCategoryshareholder' => $result[0]->catKey, 'CompanyID' => $user['CompanyID']));
			//echo $ci->db->last_query();
			$result2 = $query2->result();	
			return $result2[0]->FirstName.' '.$result2[0]->LastName;			
		} else {
			return '';
		}		
	}
	#GET Shareholder Name 
	function getEmployeeNameTBListing($title) {		
		$ci = &get_instance();
		$user = $ci->session->userdata('user');
		$prefix = $ci->db->dbprefix;
		$ci->db->select('catKey');			
		...
		if(!empty($result)){
			$ci->db->select('FirstName,LastName');
			$query2 = $ci->db->get_where('cashman_company_customers', array('TbCategoryEmployee' => $result[0]->catKey, 'CompanyID' => $user['CompanyID']));
			//echo $ci->db->last_query();
			$result2 = $query2->result();	
			return $result2[0]->FirstName.' '.$result2[0]->LastName;			
		} else {
	...
	#GET Bank Name 
	function getBankNameTBListing($title) {		
		$ci = &get_instance();
		$user = $ci->session->userdata('user');
		$prefix = $ci->db->dbprefix;
		$ci->db->select('catKey');			
		...
		if(!empty($result)){
			$ci->db->select('Name');
			$query2 = $ci->db->get_where('cashman_banks', array('TB_Category' => $result[0]->catKey, 'CompanyID' => $user['CompanyID']));
			//echo $ci->db->last_query();
			$result2 = $query2->result();	
	...
	function categoryIDTrialCat($key) {		
        if (empty($key)) {
            return FALSE;
        }
        $ci = &get_instance();
