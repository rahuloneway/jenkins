...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
             ...
             */
            $userDataGlobal = $this->_get_userdata($this->_apiuser->Email, $this->_apiuser->Password,$this->_apiuser->CompanyId);
            $this->_get_session($userDataGlobal);
     ...
     */
    protected function createKey($userData, $password, $deviceId)
    {
        // Build a new key
        $key = $this->_generate_key();        

        // Insert the new key
        if ($this->_insert_key($key, $userData, $password, $deviceId))
        {
            return $key;
        }
        else
     ...
     */
    protected function regenerateKey($userId, $deviceId)
    {
        // Build a new key
        $new_key = $this->_generate_key();

        // Insert the new key
        if ($this->_update_key($userId ,$new_key, $deviceId))
        {
            return  $new_key;
        }
        else
     ...
     */
    protected function _get_userdata($username, $password,$companyId=null)
    { 
        $this->load->model('login');
        return $this->login->clientApiLogin($username,$password,0,$companyId);
    }
	/**
     ...
     */
	protected function _get_usercompanies($clientId, $fields)
    {
        $this->load->model('login');
        return $this->login->getClientCompanies($clientId,$fields);
    }
     ...
     */
    protected function _get_key_user($userId,$DeviceId)
    {
        return $this->db
            ->where('UserId', $userId)
     ...
     */
    protected function _insert_key($key, $userData, $password, $deviceId)
    {
     ...
     */
    protected function _update_key($userId, $newKey=NULL, $deviceId)
    {
        return $this->db
	...
	protected function updateCompanyId($userId, $deviceId,$companyId=NULL)
    {
        $this->db
		...
		$this->load->model('login');
        $userDataGlobal =  $this->login->clientApiLogin($this->_apiuser->Email,$this->_apiuser->Password,0,$this->_apiuser->CompanyId);
		$this->_get_session($userDataGlobal);
    }
